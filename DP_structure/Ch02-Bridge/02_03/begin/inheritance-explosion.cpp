...
int main()
{
  ...
  EncryptedTextHandler *encryptedTextHandler = new EncryptedTextHandler();
  // Create an array of pointers to CloudStorage objects.
    const std::unique_ptr<ITextSharer> sharingServices[]
    {
        make_unique<EmailShare>(*plainTextHandler),
        // make_unique<SMSShare>(),
        make_unique<EmailShareEncrypted>(*encryptedTextHandler),
